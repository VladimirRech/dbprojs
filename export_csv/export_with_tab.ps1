...
select so.OrderID
    ...
    , sct.TransactionDate
    , format(sct.AmountExcludingTax, 'N', 'en-US') as AmountExcludingTax
    , format(sct.OutstandingBalance, 'N', 'en-US') as OutstandingBalance
    , format(sct.TransactionAmount, 'N', 'en-US') as TransactionAmount
    , sc.CustomerName
    , sc.PhoneNumber
...
$FileName = "my_cav_file_" + $Today + ".csv"
